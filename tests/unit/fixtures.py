...
def store_config_in_aws(config: CourierConfig):
    ...
    _ = ssm_client.put_parameter(
        ...
        Value=config.role_name if config.alias_name is None else config.alias_name,
        Type="String",
    )
    for descriptor, folder_to_watch in config.folders_to_watch.items():
        _ = ssm_client.put_parameter(
            Name=f"{SSM_PARAMETER_PREFIX}/{config.alias_name}/folders/{descriptor}",
...
def cleanup_config_in_aws(config: CourierConfig):
    ...
    for descriptor in config.folders_to_watch:
        _ = ssm_client.delete_parameter(Name=f"{SSM_PARAMETER_PREFIX}/{config.alias_name}/folders/{descriptor}")
